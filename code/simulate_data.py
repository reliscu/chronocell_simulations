...
def simulate_protein_from_RNA(Y, topo, true_t, true_l, phi, random_seed=0):
    ...
    y0 = Y[0, :, 1] # RNA abundance at t[0]
    ss_rate = phi[:, 0] / phi[:, -1] # Definition of steady-state protein production rate 
    ...
    for l in range(L):
        ...
        dt = np.concatenate([np.diff(t_l.ravel()), [0.0]]) # Appending 0 since y_l[n-1] gets weight 0 (no interval to its right within the trajectory) 
        y_l_dt = y_l * dt[:, None] # Multiply each timepoint's RNA by its corresponding time step size (Riemann approximation)

        # Decay of pre-existing protein
        p_decay = p0 * np.exp(-deg_rates * t_l) # Pre-existing protein that has not yet degraded

        # Decay matrix: only past intervals contribute
        # i.e. y_l[i] is sampled at time t[i] and is taken to represent the RNA level throughout [t[i], t[i+1]).
        # RNA produced at t[i+1] or later belongs to next interval.
 
        t_diff = t_l - t_l.T # t_diff[m,i] = t[m] - t[i]
        mask = (t_diff > 0) # Again, RNA at time t[i] > t[m] can't contribute protein
        # Clip to avoid computing large exp(+positive) for future entries; the mask discards them anyway, but clipping keeps  values bounded.
        decay_matrix = np.exp(-np.maximum(t_diff, 0)[:, :, None] * deg_rates) # decay_matrix[m, i, p]: decay factor for protein abundance at t[m] from RNA available at t[i] for gene p
        decay_matrix = np.where(mask[:, :, None], decay_matrix, 0) 
        
        protein_contrib = (decay_matrix * y_l_dt[None, :, :]).sum(axis=1) # Sum RNA still surviving up to each time point (i.e. each cell)
        # Note to self: protein_contrib[target_time_idx, gene_idx] = np.sum(decay_matrix[target_time, :(target_time + 1), gene_idx] * y_l_dt[None, :(target_time + 1), gene_idx])
        P[l*n:(l+1)*n] = p_decay + transl_rates * protein_contrib # Protein abundance in each cell = pre-existing protein + newly synthesized protein
...
def simulate_protein_transl_rate_sensitivity(Y, topo, true_t, true_l, phi, dispersion_factor=3, n_resamples=100, random_seed=0):
    ...
    for l in range(L):
        ...
        decay_matrix = np.exp(-t_diff[:, :, None] * deg_rates) # Decay_matrix[m, i, p] = decay factor for protein abundance at t_m (made from RNA available at t_i) for gene p
        mask = (t_diff >= 0)[:, :, None]
