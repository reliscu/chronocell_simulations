...
def backward_distribution(Y, A, X_bw, X_fwd, states, t, tau, state_grid):
    ...
    for k in reversed(range(1, len(t))):
        ...
        if state_prev == state_curr:
            dt = t_curr - t_prev
            # A_k = A[state_curr]
            # A_k_rev = reverse_generator(A_k, mu_k) 
            # x_prev = expm_multiply(A_k_rev.T * dt, x_curr) # x_curr @ sp.linalg.expm(A_k_rev * dt)
            ...
            A_k2 = A[state_curr]
            A_k2_rev = reverse_generator(A_k2, mu_k)
            x_mid = expm_multiply(A_k2_rev.T * dt2, x_curr) # x_curr @ sp.linalg.expm(A_k2_rev * dt2) 
            
            dt1 = t_s - t_prev # left interval: (t_{k-1}, state_switch_time]
            A_k1 = A[state_prev]
            A_k1_rev = reverse_generator(A_k1, mu_k) 
            x_prev = expm_multiply(A_k1_rev.T * dt1, x_mid) # x_mid @ sp.linalg.expm(A_k1_rev * dt1) 
